...
public class TagSetPose extends Command {
    @SuppressWarnings("PMD.UnusedPrivateField")


    private final CommandSwerveDrivetrain m_Drivetrain;
    private final int tagID; // reference tag id
    ...
    private final Pose2d tagToGoal; // goal from tag
    private Pose2d tagFieldPose = new Pose2d();
    private Pose2d goalFieldPose = new Pose2d();
    ...
    */
    public TagSetPose(CommandSwerveDrivetrain m_Drivetrain, int tagID, Pose2d tagToGoal) {
    ...
    public void initialize() {
        ...
        //get global pose
        Optional<Pose3d> tagPose3d = kTagLayout.getTagPose(tagID);
        if (tagPose3d.isEmpty()) {
            // handle invalid tag ID
            return;
        }
        tagFieldPose = tagPose3d.get().toPose2d();
        //Pose2d tagToGoal = new Pose2d(goalX_meters, goalY_meters, Rotation2d.fromRadians(goalThetaRad));
        //Pose2d to transform2d 
        Transform2d tagToGoalTransform2d =
        new Transform2d(
            new Translation2d(tagToGoal.getX(), tagToGoal.getY()),
            tagToGoal.getRotation()
        );
        goalFieldPose = tagFieldPose.transformBy(tagToGoalTransform2d);
